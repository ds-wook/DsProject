...
def del_columns(train: pd.DataFrame) -> pd.DataFrame:
    del_features = [
        "Real Free %",
        "Virtual free %",
        "Real total(MB)",
...
def change_column_name(train: pd.DataFrame) -> pd.DataFrame:
...
def time_split_df(train: pd.DataFrame) -> pd.DataFrame:
